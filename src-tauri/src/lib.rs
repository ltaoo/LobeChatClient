...
async fn download_zip_file_then_unzip(
    ...
    while let Some(chunk) = stream.next().await {
        ...
        println!("{}", msg);
        window.emit("data", msg).unwrap();
    }
    writer.flush()?;
    // println!("File downloaded and saved to {:?}", filepath);
    // println!("_download file finish");
    match get_file_parent_directory(&filepath) {
        Some(parent) => {
            // if is_directory_exists(&parent) {
            //     return Ok(());
...
#[tauri::command]
fn set_complete(app: AppHandle) -> Result<(), ()> {
    ...
    let main_window = WebviewWindow::builder(
        &app,
        "main",
        tauri::WebviewUrl::App("http://127.0.0.1:3000".into()),
    )
    .build()
    .unwrap();
